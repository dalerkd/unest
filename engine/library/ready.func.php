...
class ReadyFunc{
	...
	public static function compress_same_char_output($soul_usable,&$all_valid_mem_opt_index){
		...
		foreach ($soul_usable as $a => $b){
			foreach ($b as $c => $d){
				if (isset($d['prev'][FLAG_WRITE_ABLE])){
					$ret[$a][$c][P][FLAG_WRITE_ABLE] = $d['prev'][FLAG_WRITE_ABLE];
				}
				if (isset($d['prev'][NORMAL_WRITE_ABLE])){
					$ret[$a][$c][P][NORMAL_WRITE_ABLE] = $d['prev'][NORMAL_WRITE_ABLE];
				}
				if (isset($d['prev'][MEM_OPT_ABLE])){
					foreach ($d['prev'][MEM_OPT_ABLE] as $z => $y){
						if (isset($all_valid_mem_opt_record[$z][$y[BITS]][$y[OPT]])){
	...
	private static function combine_valid_mem_array($c_exec_list,$c_valid_mem_opt_tmp){
		...
		foreach ($c_exec_list as $a => $b){	    
			foreach ($b as $c => $d){
				if ('-' === $d)
					continue;
				if (!isset($history_prev[$d])){  //  第一次，直接赋值
					$history_prev[$d] = true;
					$ret[$d]['prev'] = isset($c_valid_mem_opt_tmp[$a][$d]['prev'])?$c_valid_mem_opt_tmp[$a][$d]['prev']:NULL;
				}else{               //非第一次，做merge
					if ((!isset($ret[$d]['prev'])) || (!isset($c_valid_mem_opt_tmp[$a][$d]['prev']))){
						unset ($ret[$d]['prev']);
					}else{
				...
				if ((isset($b[$c + 1])) and ('-' === $b[$c + 1])){ //jcc 跳转 的后保护被忽略，而不是禁用所有 | jmp/ret/call 之类绝对跳转不管，后一指令的前保护就等于继承了
						continue;
				}
				if (!isset($history_next[$d])){  //  第一次，直接赋值
					
					$history_next[$d] = true;
					$ret[$d]['next'] = isset($c_valid_mem_opt_tmp[$a][$d]['next'])?$c_valid_mem_opt_tmp[$a][$d]['next']:NULL;				
				}else{               //非第一次，做merge
					if ((!isset($ret[$d]['next'])) || (!isset($c_valid_mem_opt_tmp[$a][$d]['next']))){
						unset ($ret[$d]['next']);
					}else{
	...
	*/
	...
	private static function break_avoid_mem_usable_reg($obj_reg,$c_normal_register_opt){
		...
		if ($obj_reg){ //有效内存 由 寄存器 构建，当寄存器被改写，中断所有操作
			foreach ($obj_reg as $z => $y){
				if (isset($c_normal_register_opt[$y])){
	...
	private static function get_execlist_usable_mem($c_exec_thread_list,$c_valid_mem_opt_array,&$ret,$c_normal_register_opt_array){
		...
		foreach ($c_valid_mem_opt_array as $a => $b){
			...
			foreach ($objs as $c => $d){
				for ($k = 0;;$k++){
					if (!isset($c_valid_mem_opt_array[$c_exec_thread_list[$d]][$k])){
						break;
					}
				
					$obj_reg = false;
					//正常情况下，一条指令至多一个内存地址，直接[$k = 0]取
					if (isset($c_valid_mem_opt_array[$c_exec_thread_list[$d]][$k][REG])){
					...
					while ($i >= 0){ //向上遍历					
						...
						$i--;			
						if ((isset($c_exec_thread_list[$i])) and ('-' === $c_exec_thread_list[$i])){ //来源跳转					
							$i--;	
							//有效内存 由 寄存器 构建，当寄存器被改写，中断所有操作
							if (isset($c_normal_register_opt_array[$c_exec_thread_list[$i]])){
						...
						if ($opt_permission > 1){
							if (isset($c_valid_mem_opt_array[$c_exec_thread_list[$i]][$k])){
		...
		foreach ($c_valid_mem_opt_array as $a => $b){
			...
			foreach ($objs as $c => $d){
				for ($k = 0;;$k++){
					...
					while (isset($c_exec_thread_list[$i+1])){        //向下遍历，继承 可读
						...
						if ((isset($c_exec_thread_list[$i + 1])) and ('-' === $c_exec_thread_list[$i + 1])){  //来源跳转
							$i ++;
							continue;
						}
					}
	...
	public static function get_soul_usable_limit($CodeSectionArray,$exec_thread_list,$StandardAsmResultArray,$stack_used,$stack_broke){
		...
		foreach ($CodeSectionArray as $a => $b){
			...
			foreach ($exec_thread_list[$a] as $c => $d){
				//处理本流程的有效内存
				if (isset($valid_mem_opt_array[$a])){
				...
				for ($max = count($d) - 1; $max >= 0; $max--){	  // 倒序 遍历
					...
					if ($miss_next_element){
						...
						if (!isset($next_dealed_record[$exec_thread_list[$a][$c][$max]])){
					...
					//标志 寄存器
					if (isset($flag_register_opt_array[$a][$d[$max]])){
					...
					//通用寄存器
					if (isset($normal_register_opt_array[$a][$d[$max]])){
					...
					if (!isset($prev_dealed_record[$exec_thread_list[$a][$c][$max]])){
					...
					if (isset ($stack_broke[$a][$exec_thread_list[$a][$c][$max]])){					
						if (true === $stack_flag){
							$stack_flag = false;
						}else{
							$stack_buff[$max] = 2;
						...
						$stack_buff[$max] = 1;
					}elseif (isset($stack_used[$a][$exec_thread_list[$a][$c][$max]])){					
			...
			foreach ($StandardAsmResultArray[$a] as $z => $y){
				if (isset($soul_usable[$a][$z]['prev'][NORMAL_WRITE_ABLE])){
				...
				if (isset($soul_forbid[$a][$z][P][NORMAL])){
				...
				if (isset($soul_usable[$a][$z]['next'][NORMAL_WRITE_ABLE])){
				...
				if (isset($soul_forbid[$a][$z][N][NORMAL])){
	...
	private static function deal_exec_thread_list_get($c_line,$c_asm_array,$c_thread_id,$c_bound_end,$c_solid_jmp_to,&$exec_thread_list,$list_id,&$c_enumming_array,&$list_id_ptr,&$jmp_back_record,&$bound_start){
		...
		while ($c_line < $c_bound_end){
			if (isset($c_asm_array[$c_line])){
				$old = array();
				if (isset($exec_thread_list[$c_thread_id][$list_id])){
				...
				if (Instruction::isJmp($c_asm_array[$c_line][OPERATION],2)){ //绝对跳转 (备忘:ret/jmp... 后面应无需考虑执行)
																		 	  //         (         call   后面应  需考虑执行)
					$tmp = $c_line + 1;
					if ((!isset($bound_start[$tmp])) or (!$bound_start[$tmp])){					
	...
	public static function exec_thread_list_get($CodeSectionArray,$StandardAsmResultArray,&$exec_thread_list,$solid_jmp_to,$AsmLastSec){    
		foreach ($CodeSectionArray as $a => $b){
			...
			while (count($c_enumming_array)){
				...
				$tmp = isset($solid_jmp_to[$a])?$solid_jmp_to[$a]:NULL;
				self::deal_exec_thread_list_get($d,$StandardAsmResultArray[$a],$a,key($AsmLastSec[$a]),$tmp,$exec_thread_list,$c,$c_enumming_array,$list_id_ptr,$jmp_back_record,$bound_start);
			}
		...
		foreach ($CodeSectionArray as $a => $b){
			foreach ($AsmResultArray[$a] as $c => $d){
					...
					if (isset($StandardAsmResultArray[$a][$c][OPERATION])){
						...
						if (false === $c_param_type_complete){
							...
							if ('[' === $x) {
								...
								if ((!isset($c_solid_bits[$p_ptr])) or (!$c_solid_bits[$p_ptr])){
						...
						if (isset($StandardAsmResultArray[$a][$c][PARAMS][$p_ptr])){
				...
				if ($break){ //出错，放弃当前节表处理
					...
					if (isset($StandardAsmResultArray[$a][$c][PARAMS])){
						$c_instruction = Instruction::getInstructionOpt($StandardAsmResultArray[$a][$c][OPERATION],count($StandardAsmResultArray[$a][$c][PARAMS]));	
					}else{
						$c_instruction = Instruction::getInstructionOpt($StandardAsmResultArray[$a][$c][OPERATION],0);	
					}
					...
					if (true === $param_include_r_m){
						foreach ($StandardAsmResultArray[$a][$c][PARAMS] as $z => $y){
							...
							if ('r' === $StandardAsmResultArray[$a][$c][P_TYPE][$z]){ //寄存器	
								...
								if (isset($normal_register_opt_array[$a][$c][$cri][$StandardAsmResultArray[$a][$c][P_BITS][$z]])){
	...
	public static function eip_label_replacer($AsmLastSec,&$solid_jmp_array,&$solid_jmp_to,&$myTables,&$AsmResultArray,$LineNum_Code2Reloc,$language){
		...
		foreach ($myTables['CodeSectionArray'] as $a => $b){
			...
			foreach ($AsmResultArray[$a] as $c => $d){
				...
				if (!isset($LineNum_Code2Reloc[$a][$c])){ //没有重定位
					$tmp = explode (' ',$d['asm']);
					if (Instruction::isEipInst($tmp[0])){  //不考虑 前缀 指令 * 跳转指令前面 有可能有前缀吗？ 不可能()
						...
						$f = count ($tmp) - 1;
                        if (isset($tmp[$f - 1])){
							if (($tmp[$f - 1] == 'SHORT') or ($tmp[$f - 1] == 'DWORD')){
								unset ($tmp[$f - 1]);
							}
						}
	...
	public static function redeal_split_opt($StandardAsmResultArray,$exec_thread_list,&$soul_forbid,&$soul_usable){	
		foreach ($exec_thread_list as $sec => $c_exec_thread_list){
			...
			if (false !== $need){ //末尾也是个split opt
				$use_stack = false;
				if ((isset($soul_usable[$sec][$need][N][STACK]))and(true === $soul_usable[$sec][$need][N][STACK])){
					$use_stack = true;
				}
				unset ($soul_forbid[$sec][$need][N]);
				unset ($soul_usable[$sec][$need][N]);
    ...
    // 
	public static function unified_by_DList_key(){
		global $soul_writein_Dlinked_List_Total;
		global $soul_forbid; 
		global $soul_usable;
		global $StandardAsmResultArray;
		global $exec_thread_list;
		global $solid_jmp_array;
		global $solid_jmp_to;
		global $normal_register_opt_array;
		global $flag_register_opt_array;
		global $valid_mem_opt_array;

		$tmp_soul_writein_Dlinked_List_Total = $soul_writein_Dlinked_List_Total;
		$tmp_StandardAsmResultArray = $StandardAsmResultArray;
		$StandardAsmResultArray = array();
		$tmp_soul_forbid = $soul_forbid;
		$soul_forbid = array();
		$tmp_soul_usable = $soul_usable;
		$soul_usable = array();
		$tmp_exec_thread_list = $exec_thread_list;
		$exec_thread_list = array();
		$tmp_solid_jmp_array = $solid_jmp_array;
		$solid_jmp_array = array();
		$tmp_solid_jmp_to = $solid_jmp_to;
		$solid_jmp_to = array();
		$tmp_normal_register_opt_array = $normal_register_opt_array;
		$normal_register_opt_array = array();
		$tmp_flag_register_opt_array = $flag_register_opt_array;
		$flag_register_opt_array = array();
		$tmp_valid_mem_opt_array = $valid_mem_opt_array;
		$valid_mem_opt_array = array();

		foreach ($tmp_soul_writein_Dlinked_List_Total as $sec => $z){
			$tmp_map = array(); // trans map
			foreach ($z as $key => $value){				
				$tmp_map[$value[C]] = $key;
				$soul_key = $value[C];
				unset($soul_writein_Dlinked_List_Total[$sec][$key][C]);
				if (isset($soul_writein_Dlinked_List_Total[$sec][$key][LABEL])){
					$StandardAsmResultArray[$sec][$key][LABEL] = $soul_writein_Dlinked_List_Total[$sec][$key][LABEL];
					unset($soul_writein_Dlinked_List_Total[$sec][$key][LABEL]);
				}elseif (isset($tmp_StandardAsmResultArray[$sec][$soul_key])){
					$StandardAsmResultArray[$sec][$key] = $tmp_StandardAsmResultArray[$sec][$soul_key];
				}				
				if (isset($tmp_soul_forbid[$sec][$soul_key])){
					$soul_forbid[$sec][$key] = $tmp_soul_forbid[$sec][$soul_key];
				}
				if (isset($tmp_soul_usable[$sec][$soul_key])){
					$soul_usable[$sec][$key] = $tmp_soul_usable[$sec][$soul_key];
				}
				if (isset($tmp_solid_jmp_array[$sec][$soul_key])){
					$solid_jmp_array[$sec][$key] = $tmp_solid_jmp_array[$sec][$soul_key];
				}
				if (isset($tmp_normal_register_opt_array[$sec][$soul_key])){
					$normal_register_opt_array[$sec][$key] = $tmp_normal_register_opt_array[$sec][$soul_key];
				}
				if (isset($tmp_flag_register_opt_array[$sec][$soul_key])){
					$flag_register_opt_array[$sec][$key] = $tmp_flag_register_opt_array[$sec][$soul_key];
				}
				if (isset($tmp_valid_mem_opt_array[$sec][$soul_key])){
					$valid_mem_opt_array[$sec][$key] = $tmp_valid_mem_opt_array[$sec][$soul_key];
				}
			}
			foreach ($tmp_exec_thread_list[$sec] as $list_num => $z){
				foreach ($z as $tmp){
					if (isset($tmp_map[$tmp])){
						$tmp = $tmp_map[$tmp];
					}//else{'-' == $tmp}
					$exec_thread_list[$sec][$list_num][] = $tmp;
				}				
			}
			if (isset($tmp_solid_jmp_to[$sec])){
				foreach ($tmp_solid_jmp_to[$sec] as $a => $b){
					if ((isset($tmp_map[$a])) and (isset($tmp_map[$b]))){
						$a = $tmp_map[$a];
						$b = $tmp_map[$b];
						$solid_jmp_to[$sec][$a] = $b;
					}else{
						return false;
					}
				}
	...
	//
	public static function add_soul_writein_Dlinked_List(&$soul_writein_Dlinked_List,&$soul_len_array,&$num,&$prev,$label,$asm,$ia = false){
		global $label_index;
		global $sec;
		global $soul_usable;
		global $soul_forbid;
		...
		if (false !== $label){
			...
			// }else{
			// 	$c_asm = $StandardAsmResultArray[$sec][$asm];				
			...
			$soul_len_array[$sec][$num] = $AsmResultArray[$sec][$asm]['len'];			
		}
	...
	public static function generat_soul_writein_Dlinked_List(&$soul_writein_Dlinked_List,&$soul_len_array,$a,&$num,&$prev,$c_solid_jmp_array){

		if (isset($c_solid_jmp_array[$a])){ //标号
			foreach ($c_solid_jmp_array[$a] as $z => $y){
				self::add_soul_writein_Dlinked_List($soul_writein_Dlinked_List,$soul_len_array,$num,$prev,$y,$a);				
			}
		}	
		
		self::add_soul_writein_Dlinked_List($soul_writein_Dlinked_List,$soul_len_array,$num,$prev,false,$a);
